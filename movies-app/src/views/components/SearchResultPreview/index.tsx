...
function SearchResultPreview({ id, title, overview, genres, releaseDate, posterPath, mediaType }: SearchResult) {
    ...
    return (
        <div className='search_result_preview w-52 rounded'>
            <Link to={`/${mediaTypeUrl}/${id}`}>
                ...
                <div className='search_result_preview_title'>{title}</div>
                <div className='search_result_preview_release_date'>{releaseDate?.slice(0, 4)}</div>
